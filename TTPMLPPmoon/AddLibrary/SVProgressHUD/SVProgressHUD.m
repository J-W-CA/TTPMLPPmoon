...
+ (void)showOnlyText:(NSString *)status{

    [[self sharedView] showOnlyText:status];
}
- (void)showOnlyText:(NSString *)status{
    // 只显示文本
    self.isOnlyText = YES;
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.fadeOutTimer = [NSTimer timerWithTimeInterval:1.5 target:strongSelf selector:@selector(dismiss) userInfo:nil repeats:NO];
...
#pragma clang diagnostic pop
    ...
    if(_isOnlyText){
        ...
        self.hudView.bounds = CGRectMake(0.0f, 0.0f, MAX(self.minimumSize.width, hudWidth), 30);
        labelRect = CGRectMake(5, 0, MAX(self.minimumSize.width, hudWidth)-10, 30);
    }else{
